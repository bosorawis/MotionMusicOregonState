...
public class SeekBarPreference extends DialogPreference {
	...
	protected final View onCreateDialogView() {
		...
		TextView progressTextView = (TextView) dialogView
				.findViewById(R.id.progress_text);
		progressTextView.setText(getProgressText());
		progressTextView.setVisibility(isProgressShown() ? View.VISIBLE
				: View.GONE);

		TextView dialogMessageTextView = (TextView) dialogView
				.findViewById(R.id.dialog_message);
		dialogMessageTextView.setText(getDialogMessage());
	...
	protected final Parcelable onSaveInstanceState() {
		...
		SeekBarPreferenceSavedState savedState = new SeekBarPreferenceSavedState(
				superState);
		savedState.setValue(value);
		savedState.setSeekBarValue(seekBarValue);
		savedState.setMinValue(minValue);
		savedState.setMaxValue(maxValue);
		savedState.setStepSize(stepSize);
		savedState.setDecimals(decimals);
		savedState.setSuffix(suffix);
		savedState.setFloatingPointSeparator(floatingPointSeparator);
		savedState.showValueAsSummary(showValueAsSummary);
		savedState.showProgress(showProgress);
		savedState.setSummaries(summaries);
		return savedState;
	}

	@Override
	protected final void onRestoreInstanceState(final Parcelable state) {
		if (state != null && state instanceof SeekBarPreferenceSavedState) {
			SeekBarPreferenceSavedState savedState = (SeekBarPreferenceSavedState) state;
			value = savedState.getValue();
			seekBarValue = savedState.getSeekBarValue();
			minValue = savedState.getMinValue();
			maxValue = savedState.getMaxValue();
			stepSize = savedState.getStepSize();
			decimals = savedState.getDecimals();
			suffix = savedState.getSuffix();
			floatingPointSeparator = savedState.getFloatingPointSeparator();
			showValueAsSummary = savedState.isValueShownAsSummary();
			showProgress = savedState.isProgressShown();
			summaries = savedState.getSummaries();
